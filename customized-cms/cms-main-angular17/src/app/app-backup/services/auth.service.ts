...
import { needLogin, /* later Roles,userList, users*/ } from 'src/app/app-backup/settings-and-models/static-text-other-constants';
...
export class AuthService {
  ...
  */
  ...
  /* Angular 17 migration and optimization. Login returns after authentication */
  login(username:string='default', directLatet=false){
    this.clearCaches() ;
    let loggedinUser = username ==='default'? 
          { userName: 'default', userRoles: ["any"] } :
          this.userList.find((userObject) => userObject.userName === username);
    const cacheLoggedInUser  = JSON.stringify(loggedinUser) ;
    console.log(`Auth service ${cacheLoggedInUser}, ${username}`);
  ...
  /* Pre-angular 17 login logic */
  loginAndRedirect(username:string='', directLatet=false){
    console.log('kogib') ;
    /*
    ...
    if (directLatet === true) {
      this.router.navigate(['/view','latest-uploads-timewise']).then( (e) => {
        ...
        if (e) {
          ...
          this.router.navigate(['/view','top-level']);
        }
      });
  } this.router.navigate(['/view','top-level']);
  }
