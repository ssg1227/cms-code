import {  ImageElement } from '@settings-and-models/image-detail'

export class CreamCakesOreoBfWfWan {
            sort: string = 'asc'
    imageRoot=`assets/gallery-files/images/cream-cakes-oreo-bf-wf-van//`;
    public allImageList:ImageElement[]  =  [
          { 
                folder:'cream-cakes-oreo-bf-wf-van',
                theme:
                'Scenes, Objects, Structures..',
                themeSummary:`</strong>Scenes, Objects, Structures, (some may be items containing mix modes of transport or something like an air dogfight)<br/>
                Mumbai gets her due place and a dedicated theme; all Mumbai related sketches will be found in 'Mumbai Meri Jaan'`,
                files: [ 
{
canvassSize:  '',
content:  '',
summaryLabel:  `Oreo Cake`,
                        dateUploaded:  '07-17-2024',
fullFileName: `${this.imageRoot}Oreo-cake-07-17.jpeg`,
description: `<ul></ul>`,
},
                  {
                        
                        
                        dateUploaded:  '06-06-2024',
                        summaryLabel:  `Oreo Cake another pic`,
                        fullFileName: `${this.imageRoot}oreo cake.jpeg`,
                        description: `<ul><li>Oreo Cake </li><li>More details coming soon </li></ul>`,
                        iterations: [
                                {fullFileName: `${this.imageRoot}oreo cake.jpeg`
                              , description: `<ul><li>Oreo Cake </li><li>More details coming soon </li></ul>
                              ` },
                              {fullFileName: `${this.imageRoot}Oreo Cake.jpg`
                              , description: `<ul><li>Oreo Cake </li><li>More details coming soon </li></ul>
                              ` },
                              ]

                  },{
                        
                        dateUploaded:  '06-06-2024',
                        summaryLabel:  `Black Forest`,
                        fullFileName: `${this.imageRoot}Vrudhi.jpeg`,
                        description: `<ul><li> Black Forest Cake  </li><li>More details coming soon </li></ul>`,
                       

                  },{
                        
                        dateUploaded:  '06-06-2024',
                        summaryLabel:  `Black Forest`,
                        fullFileName: `${this.imageRoot}black-forest-2.jpeg`,
                        description: `<ul><li> Black Forest Cake  </li><li>More details coming soon </li></ul>`,
                       

                  }
              
             ]
            },
       ]
  }
  
